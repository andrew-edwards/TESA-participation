...
funds_table = rbind(funds,
  ...
  select(-c("Division", "First.name")) # "Taken.spot"

# `Cost' is the cost of the course,
fundsTab = xtable(funds_table,
 caption="Funding available for each person. `Subsidy' is  the funding from TESA.",
 label="tab:funds",
 digits = 0)
@
...
Divions of people not in StAR or ESD:

<<>>=
funds_other_div <- filter(funds,
                          !(Division %in% c("ESD", "StAR")))
