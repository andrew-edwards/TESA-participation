...
fundsTab = xtable(funds_table,
 caption="Funding available for each person. `Cost' is the cost of the course, `Subsidy' is  the funding from TESA. The `PR.Bayesian' course corresponds to both the Introduction course and the Stan course (so some people appear twice).",
 label="tab:funds",
 digits = 0)
@
